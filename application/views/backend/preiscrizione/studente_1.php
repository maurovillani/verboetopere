...
function ConfermaInvio() {
    ...
    <?php if (intval($tab_studente['TAB'])>2): ?>    
        ...
        if (document.myForm.CORSOLAUREA.value == "210" && document.myForm.INDIRIZZOLAUREA.value == ""){
            alert("Inserire l'indirizzo di laurea");
            document.myForm.INDIRIZZOLAUREA.focus();
            return false;
        }
    ...
    <?php if (intval($tab_studente['TAB'])>=6): ?>    
        if (document.myForm.CERTNASC.value == "S"){
            ...
            <?php if ($tab_studente['CERTNASC_TIPO']!=''): ?> 
                if (document.myForm.PRIMALINGUA.value == "0"){
                    alert("Specificare la prima lingua");
                    document.myForm.PRIMALINGUA.focus();
                    return false;
                }
                if (document.myForm.SECLINGUA.value == "0") {
                    if(document.myForm.CORSOLAUREA.value != "999"){ 
                        alert("Specificare la seconda lingua");
                        document.myForm.SECLINGUA.focus();
                        return false;
                    }
                    if(document.myForm.CORSOLAUREA.value == "999" && document.myForm.CheckStraordinarioOspite.value=='1'){
                        alert("Specificare la seconda lingua");
                        document.myForm.SECLINGUA.focus();
                        return false;
                    }
                }
                if (document.myForm.TERLINGUA.value == "0") {
                    if(document.myForm.CORSOLAUREA.value < "250"){ 
                        alert("Specificare la terza lingua");
                        document.myForm.TERLINGUA.focus();
                        return false;
                    }
                }
                <?php if ($tab_studente['CITTADI2']!='1'): ?> 
                    if (document.myForm.ITASTRANIERI.value != "S") {
                        alert("E' obbligatorio avere superato QCER-A2 italiano per stranieri");
                        document.myForm.ITASTRANIERI.focus();
                        return false;
                    }
    ...
    if (document.myForm.CODFISCA.value == "") {
        alert("Attenzione: Non hai inserito il Codice Fiscale. Potrai comunque completare i dati mancanti in seguito");
    }
    if (document.myForm.TAB.value > 3 && document.myForm.STATOCIV.value != "5" && document.myForm.STATOCIV.value != "6" && document.myForm.SEMAIL.value == ""){
        alert("Attenzione: Non hai inserito l'email del Superiore/Vescovo. Potrai comunque completare i dati mancanti in seguito");
    }
//    if (document.myForm.TAB.value > 5 && document.myForm.CERTNASC.value == "S" && document.myForm.PRIMALINGUA.value != "" && document.myForm.ITASTRANIERI.value != "S" ){
//        alert("Attenzione: Non hai inserito il Cert. QCER-A2 italiano per stranieri. Potrai comunque completare i dati mancanti in seguito");
//    }
    <?php if ($tab_studente['TAB']>5):?>                    
    if (document.myForm.TITOLOSTUDIO_PDF.value != "S" 
        ...
        alert("Attenzione: Non hai inserito tutti i documenti richiesti. Potrai comunque completare i dati mancanti in seguito");
    }
    <?php endif; ?>  
//    if (document.myForm.CODFISCA.value == ""
...
<?php if (intval($tab_studente['TAB'])>3): ?>
                <li class="nav-item">
                    <a class="<?php echo ($_SESSION['tab_attivo_studente']==='tab4'? 'nav-link active':'nav-link'); ?>" id="<?php echo $tab4['id'] ?>-tab" data-toggle="tab" href="#<?php echo $tab4['id'] ?>" role="tab" aria-controls="home"  aria-selected="false" onclick="tab4()">
                        <?php echo $tab4['name'] ?>
                    </a>
                </li>
<?php endif; ?>
